<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Event Security - IslandShield Security</title>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
  <!-- Modular CSS -->
  <link rel="stylesheet" href="assets/css/base.css">
  <link rel="stylesheet" href="assets/css/layout.css">
  <link rel="stylesheet" href="assets/css/components.css">
  <link rel="stylesheet" href="assets/css/pages/pages.css">
</head>
<body>

<?php include "includes/header.php"; ?>

<section class="page-header service-header">
  <div class="container">
    <h1>Professional Event Security</h1>
    <p>Expert security teams for concerts, conferences, weddings, and corporate events</p>
  </div>
</section>

<!-- Service Overview - Split Layout -->
<section class="split-section">
  <div class="container">
    <div class="split-container">
      <div class="split-content">
        <h2>Make Your Event Safe & Successful</h2>
        <p>Whether you're hosting a small private gathering or a large public event, professional security is essential for success.</p>
        <ul>
          <li>Experienced event security teams</li>
          <li>Crowd management specialists</li>
          <li>Entry and exit control</li>
          <li>VIP protection available</li>
          <li>Emergency response ready</li>
        </ul>
        <a href="#packages" class="btn">View Event Packages</a>
      </div>
      <div class="split-visual">
        <img src="assets/images/event_security2.png" alt="Event Security">
      </div>
    </div>
  </div>
</section>

<section class="service-features-section">
  <div class="container">
    <h2 class="section-title">Event Security Services</h2>
    <div class="features-grid-detailed">
      <div class="feature-card-large">
        <div class="feature-icon-large">👥</div>
        <h3>Crowd Management</h3>
        <p>Professional crowd control and flow management to prevent overcrowding and ensure safety.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🎟️</div>
        <h3>Entry & Exit Control</h3>
        <p>Ticket verification, guest list management, and controlled access points.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🔍</div>
        <h3>Security Screening</h3>
        <p>Bag checks, metal detectors, and security screening for all attendees.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">⭐</div>
        <h3>VIP Protection</h3>
        <p>Personal security details for high-profile guests and speakers.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">📹</div>
        <h3>Surveillance Monitoring</h3>
        <p>Live camera monitoring and recording throughout your event.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🚨</div>
        <h3>Emergency Response</h3>
        <p>Trained personnel ready to handle any emergency situation.</p>
      </div>
    </div>
  </div>
</section>

<!-- Event Types -->
<section class="service-features-section">
  <div class="container">
    <h2 class="section-title">Events We Secure</h2>
    <p class="section-subtitle">Professional security for all types of events</p>
    
    <div class="features-grid-detailed">
      <div class="feature-card-large">
        <div class="feature-icon-large">🎵</div>
        <h3>Concerts & Festivals</h3>
        <p>Large-scale crowd management, stage security, and perimeter control.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">💼</div>
        <h3>Corporate Events</h3>
        <p>Professional security for conferences, seminars, and business gatherings.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">💒</div>
        <h3>Weddings & Private Parties</h3>
        <p>Discreet security for weddings, birthday parties, and celebrations.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🏃</div>
        <h3>Sports Events</h3>
        <p>Security for tournaments, races, and sporting competitions.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🎨</div>
        <h3>Cultural Events</h3>
        <p>Protection for art exhibitions, theater, and cultural celebrations.</p>
      </div>
      
      <div class="feature-card-large">
        <div class="feature-icon-large">🎓</div>
        <h3>Educational Events</h3>
        <p>Security for graduations, school events, and academic ceremonies.</p>
      </div>
    </div>
  </div>
</section>

<!-- Packages -->
<section class="packages-section">
  <div class="container">
    <h2 class="section-title">Event Security Packages</h2>
    <p class="section-subtitle">Tailored security solutions for your event size and needs</p>
    
    <div class="packages-grid">
      <div class="package-card">
        <div class="package-header">
          <h3>Small Events</h3>
          <div class="package-price">
            <span class="price-amount">$500+</span>
            <span class="price-period">Up to 100 guests</span>
          </div>
        </div>
        <ul class="package-features">
          <li>✓ 2-4 Security Officers</li>
          <li>✓ Entry Point Control</li>
          <li>✓ Guest List Management</li>
          <li>✓ 4-6 Hour Coverage</li>
          <li>✓ Incident Reporting</li>
          <li>✓ Basic First Aid</li>
        </ul>
        <a href="contact.php" class="btn">Book Now</a>
      </div>
      
      <div class="package-card featured">
        <div class="package-badge">Most Popular</div>
        <div class="package-header">
          <h3>Medium Events</h3>
          <div class="package-price">
            <span class="price-amount">$1,500+</span>
            <span class="price-period">100-500 guests</span>
          </div>
        </div>
        <ul class="package-features">
          <li>✓ 6-10 Security Officers</li>
          <li>✓ Security Supervisor</li>
          <li>✓ Crowd Management</li>
          <li>✓ Multiple Entry Points</li>
          <li>✓ 6-10 Hour Coverage</li>
          <li>✓ Security Screening</li>
          <li>✓ Emergency Response Plan</li>
          <li>✓ Radio Communication</li>
        </ul>
        <a href="contact.php" class="btn">Book Now</a>
      </div>
      
      <div class="package-card">
        <div class="package-header">
          <h3>Large Events</h3>
          <div class="package-price">
            <span class="price-amount">Custom</span>
            <span class="price-period">500+ guests</span>
          </div>
        </div>
        <ul class="package-features">
          <li>✓ 15+ Security Officers</li>
          <li>✓ Command Center Setup</li>
          <li>✓ CCTV Monitoring</li>
          <li>✓ VIP Protection Available</li>
          <li>✓ Full Day Coverage</li>
          <li>✓ Metal Detectors</li>
          <li>✓ Medical Personnel</li>
          <li>✓ Police Liaison</li>
          <li>✓ Post-Event Security</li>
        </ul>
        <a href="contact.php" class="btn">Contact Us</a>
      </div>
    </div>
  </div>
</section>

<!-- Planning Process -->
<section class="process-section">
  <div class="container">
    <h2 class="section-title">Event Security Planning Process</h2>
    <div class="process-timeline">
      <div class="process-step-detailed">
        <div class="step-number-large">1</div>
        <h3>Initial Consultation</h3>
        <p>We discuss your event details, venue, and security concerns.</p>
      </div>
      
      <div class="process-step-detailed">
        <div class="step-number-large">2</div>
        <h3>Venue Assessment</h3>
        <p>Our team visits to identify entry points and potential risks.</p>
      </div>
      
      <div class="process-step-detailed">
        <div class="step-number-large">3</div>
        <h3>Security Plan</h3>
        <p>Comprehensive plan including personnel placement and procedures.</p>
      </div>
      
      <div class="process-step-detailed">
        <div class="step-number-large">4</div>
        <h3>Team Briefing</h3>
        <p>All personnel receive detailed briefings on roles and protocols.</p>
      </div>
      
      <div class="process-step-detailed">
        <div class="step-number-large">5</div>
        <h3>Event Execution</h3>
        <p>Professional security maintained throughout your event.</p>
      </div>
      
      <div class="process-step-detailed">
        <div class="step-number-large">6</div>
        <h3>Post-Event Report</h3>
        <p>Detailed report of all security activities and incidents.</p>
      </div>
    </div>
  </div>
</section>

<!-- Why Choose Us - Card Grid -->
<section class="card-grid-section">
  <div class="container">
    <div class="card-grid-header">
      <h2>Why Choose IslandShield for Event Security?</h2>
      <p>Professional teams that make your event safe and memorable</p>
    </div>
    <div class="premium-card-grid">
      <div class="premium-card">
        <div class="premium-card-icon">🎯</div>
        <h3>Experienced Event Teams</h3>
        <p>Our personnel have secured hundreds of events across Grenada, from intimate gatherings to large festivals with thousands of attendees.</p>
      </div>
      
      <div class="premium-card">
        <div class="premium-card-icon">📋</div>
        <h3>Detailed Planning</h3>
        <p>We create customized security plans for each event, considering venue layout, guest count, and all unique factors and requirements.</p>
      </div>
      
      <div class="premium-card">
        <div class="premium-card-icon">👔</div>
        <h3>Professional Appearance</h3>
        <p>Our uniformed officers maintain a professional, approachable presence that enhances your event atmosphere while ensuring safety.</p>
      </div>
      
      <div class="premium-card">
        <div class="premium-card-icon">⚡</div>
        <h3>Rapid Response</h3>
        <p>Trained to handle emergencies quickly and efficiently without disrupting your event or alarming guests unnecessarily.</p>
      </div>
    </div>
  </div>
</section>

<!-- CTA -->
<section class="cta">
  <div class="container">
    <h2>Planning an Event?</h2>
    <p>Let us handle the security so you can focus on making your event memorable.</p>
    <div class="cta-buttons">
      <a href="contact.php" class="btn">Request Event Security Quote</a>
      <a href="tel:4735557233" class="btn btn-secondary">Call Now</a>
    </div>
    <p class="cta-note">Experienced event teams | Custom security plans | All event sizes</p>
  </div>
</section>

<?php include "includes/footer.php"; ?>


<script src="/assets/js/script.js"></script>
</body>
</html>
